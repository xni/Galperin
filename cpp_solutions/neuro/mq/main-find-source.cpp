...
const double DELTA2 = 1000000;
const double ACCURACY = 1e-3;//1e-9;

const int DIMENSIONS = 1;
const int VECTOR_SIZE = (2 + DIMENSIONS) * NEURONS;
const int VECTORS = 2 * VECTOR_SIZE;

const double W_MIN = -300;
const double W_MAX = 300;
const double A_MIN = 0.0;
const double A_MAX = right_x + top_y - left_x - bottom_y;

const double C_MIN[DIMENSIONS] = {0};
const double C_MAX[DIMENSIONS] = {0.02};

double box_points[VECTORS][VECTOR_SIZE];
double test_points[ALL_POINTS][DIMENSIONS];
double sf_inner_points[SF_INNER_POINTS][DIMENSIONS];
double sf_inner_values[SF_INNER_POINTS];
double cached_values[VECTORS];
...
void generate_test_points() {
  ...
  for (int j = 0; j < BORDER_SD_POINTS; i++,j++) {
...
void generate_sf_inner_points()
{
  for (int i = 0; i < SF_INNER_POINTS; i++) {
    sf_inner_points[i][0] = left_x + 0.1 + (right_x - left_x - 0.2) * double(rand()) / RAND_MAX;
    sf_inner_points[i][1] = bottom_y + 0.1 + (top_y - bottom_y - 0.2) * double(rand()) / RAND_MAX;
  }
}
