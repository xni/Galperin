...
#include <iomanip>
#include <fstream>
#include "../../../curve_equation/equation_circle.h"
...
const int ALL_POINTS = POINTS_INNER + 2 * POINTS_BORDER;
...
void generate_test_points() 
{
  ...
  for (int j = 0; j < POINTS_INNER; i++,j++)
  {
    ...
    if (fabs(test_points[i][0]) < 1.0)
    {
    	test_points[i][1] = pow(-1.0, rand() % 2) * (sqrt(1.0 - sqr(test_points[i][0])) + \
    		(sqrt(4.0 - sqr(test_points[i][0])) - sqrt(1.0 - sqr(test_points[i][0]))) * double(rand()) / RAND_MAX) ;
    }
    else
    {
    	test_points[i][1] = -1.0*sqrt(4.0 - sqr(test_points[i][0])) + 2.0 * sqrt(4.0 - sqr(test_points[i][0])) * double(rand()) / RAND_MAX ;
    }
  }
  for (int j = 0; j < POINTS_BORDER; i++,j++) // INSIDE BORDER
  {
    test_points[i][0] = -1.0 + 2.0 * double(rand()) / RAND_MAX;
    test_points[i][1] = sqrt(1.0 - sqr(test_points[i][0])) * pow(-1.0, rand() % 2);
  }
  for (int j = 0; j < POINTS_BORDER; i++,j++) // OUTSIDE BORDER
  {
    test_points[i][0] = left_x + (right_x - left_x)*double(rand()) / RAND_MAX;
    test_points[i][1] = sqrt(4.0 - sqr(test_points[i][0])) * pow(-1.0, rand() % 2);
  }
}
  ...
  for (int i = 0; i < POINTS_INNER; i++) 
  {
    ...
    for (int i = POINTS_INNER; i < POINTS_INNER + POINTS_BORDER; ++i)
    {
        res += DELTA * sqr(calc(a, test_points[i][0], test_points[i][1]) \
            - inside_border(test_points[i][0], test_points[i][1]));
    }
    for (int i = POINTS_INNER + POINTS_BORDER; i < POINTS_INNER + 2 * POINTS_BORDER; ++i)
    {
        res += DELTA * sqr(calc(a, test_points[i][0], test_points[i][1]) \
            - outside_border(test_points[i][0], test_points[i][1]));
    }
  return res;
}
