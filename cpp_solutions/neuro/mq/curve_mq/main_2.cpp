...
#include <iomanip>
#include <fstream>
#include "../../../curve_equation/equation2.h"
...
void generate_test_points() 
{
  ...
  {
    test_points[i][0] = double(rand() / 2.0) / RAND_MAX;
    test_points[i][1] = (double(rand()) / RAND_MAX) * sqrt(0.5 - sqr(test_points[i][0]));
  }
  for (int j = 0; j < POINTS_BORDER; i++,j++) // LEFT BORDER
  {
    test_points[i][0] = left_x;
    test_points[i][1] = bottom_y + (((sqrt(2.0) - 1.0) * rand()) / RAND_MAX) / 2.0;
  }
  for (int j = 0; j < POINTS_BORDER; i++,j++) // BOTTOM BORDER
  {
    test_points[i][0] = left_x + (((sqrt(2.0) - 1.0) * rand()) / RAND_MAX) / 2.0;
    test_points[i][1] = bottom_y;
  }
  for (int j = 0; j < POINTS_BORDER; i++,j++) // TOP BORDER
  {
    test_points[i][0] = double(rand() / 2.0) / RAND_MAX;
    test_points[i][1] = sqrt(0.5 - sqr(test_points[i][0]));
  }
}
