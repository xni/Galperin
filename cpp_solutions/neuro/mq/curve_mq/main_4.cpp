...
#include <iomanip>
#include <fstream>
#include "../../../curve_equation/equation4.h"
...
void generate_test_points() 
{
  ...
  for (int j = 0; j < POINTS_INNER; i++,j++) 
  {
    ...
    test_points[i][1] = (double(rand()) / RAND_MAX) * P_const*sqrt(1.0 - 2.0 * test_points[i][0]);
  }
  ...
  {
    test_points[i][0] = (right_x - left_x) * (double(rand()) / RAND_MAX);
    test_points[i][1] = P_const * sqrt(1.0 - 2.0 * test_points[i][0]);
  }
}
