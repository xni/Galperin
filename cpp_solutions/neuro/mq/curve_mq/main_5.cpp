...
#include <iomanip>
#include <fstream>
#include "../../../curve_equation/equation5.h"
...
void generate_test_points() 
{
  ...
  {
    test_points[i][0] = (right_x - left_x) * (double(rand()) / RAND_MAX);
    test_points[i][1] = sqrt(sqr(b_const) * (sqr(test_points[i][0]) / sqr(0.5) - 0.5)) * (double(rand()) / RAND_MAX);
  }
  for (int j = 0; j < POINTS_BORDER; i++,j++) // LEFT BORDER
  {
    test_points[i][0] = left_x;
    test_points[i][1] = (top_y - bottom_y) * (double(rand()) / RAND_MAX);
  }
  ...
  {
    test_points[i][0] = (right_x - left_x) * (double(rand()) / RAND_MAX);
    test_points[i][1] = sqrt(sqr(b_const) * ( sqr(test_points[i][0]) / sqr(0.5) - 0.5) );
  }
}
