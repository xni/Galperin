...
#include <iomanip>
#include <fstream>
#include "../../../curve_equation/equation3.h"
...
const int ALL_POINTS = POINTS_INNER + 3 * POINTS_BORDER;
...
void generate_test_points() 
{
  ...
  {
    test_points[i][0] = left_x + (double(rand()) / RAND_MAX) * (right_x - left_x);
    test_points[i][1] = sqrt((sqr(test_points[i][0]) - 0.5) / (2.0 * sqrt(2.0) - 1.0));
    test_points[i][1] += (double(rand()) / RAND_MAX) * (top_y - test_points[i][1]);
  }
  for (int j = 0; j < POINTS_BORDER; i++,j++) // BOTTOM BORDER
  {
    test_points[i][0] = left_x + (double(rand()) / RAND_MAX) * (right_x - left_x);
    test_points[i][1] = sqrt((sqr(test_points[i][0]) - 0.5) / (2.0 * sqrt(2.0) - 1.0));
  }
  for (int j = 0; j < POINTS_BORDER; i++,j++) // RIGHT BORDER
  {
    test_points[i][0] = left_x + (double(rand()) / RAND_MAX) * (right_x - left_x);
    test_points[i][1] = bottom_y;
  }
  for (int j = 0; j < POINTS_BORDER; i++,j++) // TOP BORDER
  {
    test_points[i][0] = right_x;
    test_points[i][1] = bottom_y + (double(rand()) / RAND_MAX) * (top_y - bottom_y);
  }
}
