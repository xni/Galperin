...
#include <iomanip>
#include <fstream>
#include "../../../curve_equation/equation_round.h"
...
const int ALL_POINTS = POINTS_INNER + POINTS_BORDER;
...
void generate_test_points() 
{
  ...
  for (int j = 0; j < POINTS_INNER; i++,j++) 
  {
    ...
    test_points[i][1] = -1.0*sqrt(1-sqr(test_points[i][0])) + 2.0 * sqrt(1.0 - sqr(test_points[i][0])) * double(rand()) / RAND_MAX ;
  }
  for (int j = 0; j < POINTS_BORDER; i++,j++) // BORDER
  {
    test_points[i][0] = left_x + (right_x - left_x)*double(rand()) / RAND_MAX;
