...
const int ALL_POINTS = POINTS_INNER;
...
double box_points[2 * NEURONS * 3][3 * NEURONS];
double test_points[ALL_POINTS][1];
double cached_values[2 * NEURONS * 3];

inline double calc_neuron(double *a, double x) 
{
  double x_d = fabs(x - a[1]);
  return a[0] * sqrt(x_d * x_d + a[2]*a[2]);
}
...
double laplace(double *a, double x) 
{
  ...
  {
    double dx = fabs(x - a[3 * i + 1]);
    res += a[3 * i] * (1.0 / sqrt(dx*dx + a[3 * i + 2]*a[3 * i + 2]) - dx*dx / pow(dx*dx + a[i * 3 + 2]*a[i * 3 + 2], 1.5));
  }
  return res;
}
...
{
  for (int c = 0; c < 2 * NEURONS * 3; c++) 
  { 
    for (int i = 0; i < NEURONS; i++) 
    {
      box_points[c][3 * i] = W_MIN + (W_MAX - W_MIN) * double(rand()) / RAND_MAX;
      box_points[c][3 * i + 1] = C_X_MIN + (C_X_MAX - C_X_MIN) * double(rand()) / RAND_MAX;
      box_points[c][3 * i + 2] = A_MIN + (A_MAX - A_MIN) * double(rand()) / RAND_MAX;
    }
  }
}

void fill_cache() {
  for (int i = 0; i < 2 * NEURONS * 3; i++) {
...
int box_method() 
{
  while (true) 
  {
    ...
    double cog[3 * NEURONS] = {0};
    for (int i = 0; i < 3 * NEURONS; ++i) 
    {
      for (int j = 0; j < 2 * NEURONS * 3; ++j) 
      {
    ...
    for (int i = 0; i < 10; ++i) 
    {
      double new_point[3 * NEURONS];
      for (int j = 0; j < 3 * NEURONS; ++j) 
      {
